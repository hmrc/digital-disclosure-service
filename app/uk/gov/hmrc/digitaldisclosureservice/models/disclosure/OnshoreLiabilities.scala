...
final case class OnshoreLiabilities(
  ...
  def toXml: NodeSeq =
    <onshoreLiabilities>
      ...
      {
      whatLiabilities
        .map(liabilities => <whatLiabilities>
      ...
      {
      lettingProperties
        .map(properties => <lettingProperties>
        {properties.map(property => <property>{property.toXml}</property>)}
      </lettingProperties>)
        .getOrElse(NodeSeq.Empty)
    }
      {
      memberOfLandlordAssociations
        .map(member => <memberOfLandlordAssociations>{member}</memberOfLandlordAssociations>)
        .getOrElse(NodeSeq.Empty)
    }
      {
      landlordAssociations
        .map(associations => <landlordAssociations>{associations}</landlordAssociations>)
        .getOrElse(NodeSeq.Empty)
    }
      {howManyProperties.map(count => <howManyProperties>{count}</howManyProperties>).getOrElse(NodeSeq.Empty)}
      {
      corporationTaxLiabilities
        .map(liabilities => <corporationTaxLiabilities>
        {liabilities.map(liability => <liability>{liability.toXml}</liability>)}
      </corporationTaxLiabilities>)
        .getOrElse(NodeSeq.Empty)
    }
      ...
      directorLoanAccountLiabilities
        .map(liabilities => <directorLoanAccountLiabilities>
        {liabilities.map(liability => <liability>{liability.toXml}</liability>)}
      </directorLoanAccountLiabilities>)
        .getOrElse(NodeSeq.Empty)
    }
