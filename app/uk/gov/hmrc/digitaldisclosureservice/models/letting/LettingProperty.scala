...
final case class LettingProperty(
  ...
  def toXml: NodeSeq =
    <lettingProperty>
      ...
      {
      percentageIncomeOnProperty
        .map(p => <percentageIncomeOnProperty>{p}</percentageIncomeOnProperty>)
      ...
      {
      wasPropertyManagerByAgent
        .map(flag => <wasPropertyManagerByAgent>{flag}</wasPropertyManagerByAgent>)
        .getOrElse(NodeSeq.Empty)
    }
      {
      didTheLettingAgentCollectRentOnYourBehalf
        .map(flag => <didTheLettingAgentCollectRentOnYourBehalf>{flag}</didTheLettingAgentCollectRentOnYourBehalf>)
        .getOrElse(NodeSeq.Empty)
    }
    </lettingProperty>
}
...
object LettingProperty {
  implicit val format: Format[LettingProperty] = Json.format[LettingProperty]
}
