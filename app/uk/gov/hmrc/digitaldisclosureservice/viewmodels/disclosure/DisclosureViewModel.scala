...
object DisclosureViewModel extends SummaryListFluency with SubmissionViewModel {
  ...
  def apply(fullDisclosure: FullDisclosure, caseflowDateFormat: Boolean)(implicit
    messages: Messages
  ): DisclosureViewModel = {
    ...
    val entity =
      fullDisclosure.personalDetails.background.disclosureEntity.map(_.entity.toString).getOrElse("Individual")
    ...
    val onshoreLiabilities = {
      ...
      if (disclosingOnshore)
        fullDisclosure.onshoreLiabilities.map(l =>
    ...
    DisclosureViewModel(
      metadataList(
        ...
        fullDisclosure.metadata,
        fullDisclosure.caseReference,
        fullDisclosure.offshoreLiabilities,
        caseflowDateFormat
      ),
      backgroundList(fullDisclosure.personalDetails.background),
      fullDisclosure.personalDetails.aboutTheIndividual.map(aboutTheIndividual =>
      ...
      fullDisclosure.personalDetails.aboutTheEstate.map(aboutTheEstate =>
        aboutTheEstateList(aboutTheEstate, caseflowDateFormat)
      ),
      aboutYouList(
        fullDisclosure.personalDetails.aboutYou,
        fullDisclosure.personalDetails.background,
        fullDisclosure.disclosingAboutThemselves,
        caseflowDateFormat
      ),
      ...
      additionalInformationList(
        fullDisclosure.reasonForDisclosingNow,
        fullDisclosure.disclosingAboutThemselves,
        entity
      ),
  ...
  def metadataList(
    ...
    offshore: OffshoreLiabilities,
    caseflowDateFormat: Boolean
  )(implicit messages: Messages): SummaryList = {
    val rows = Seq(
      metadata.reference.map(ref => row(s"disclosure.metadata.reference", ref)),
      metadata.submissionTime.map(time =>
        row(s"disclosure.metadata.submissionTime", toPrettyDate(time, caseflowDateFormat))
      ),
  ...
  def backgroundList(background: Background)(implicit messages: Messages): SummaryList = SummaryListViewModel(
    rows = Seq(
      background.disclosureEntity.map(answer =>
        row("disclosure.background.disclosureEntity", messages(s"notification.background.${answer.entity.toString}"))
      ),
  ...
  def otherLiabilitiesList(otherLiabilities: OtherLiabilities, disclosingAboutThemselves: Boolean, entity: String)(
    ...
    rows = Seq(
      ...
      otherLiabilities.taxCreditsReceived.map(answer =>
        taxCreditsReceivedRow(answer, disclosingAboutThemselves, entity)
      )
    ).flatten
  )

  def taxCreditsReceivedRow(received: Boolean, disclosingAboutThemselves: Boolean, entity: String)(implicit
    messages: Messages
  ) =
    if (disclosingAboutThemselves)
      SummaryListRowViewModel("disclosure.otherLiabilities.taxCredits.you", ValueViewModel(received))
    else if (entity == "Individual")
      SummaryListRowViewModel("disclosure.otherLiabilities.taxCredits.individual", ValueViewModel(received))
  ...
  def additionalInformationList(reason: ReasonForDisclosingNow, disclosingAboutThemselves: Boolean, entity: String)(
    implicit messages: Messages
  ) = SummaryListViewModel(
    rows = Seq(
      ...
      reason.receivedAdvice.map(answer =>
        SummaryListRowViewModel(
          if (disclosingAboutThemselves) "disclosure.additional.adviceGiven.you"
          else s"disclosure.additional.adviceGiven.$entity",
          ValueViewModel(answer)
        )
      ),
      reason.personWhoGaveAdvice.map(answer => row("disclosure.additional.advice.name", answer)),
      reason.adviceOnBehalfOfBusiness.map(answer =>
        SummaryListRowViewModel("disclosure.additional.advice.behalfOf", ValueViewModel(answer))
      ),
      ...
      reason.adviceGiven.map(answer =>
        SummaryListRowViewModel(
          "disclosure.additional.advice.discuss",
          ValueViewModel(answer.contactPreference != AdviceContactPreference.No)
        )
      ),
  ...
  def totalAmountsSummaryList(totalAmounts: TotalAmounts, offerAmount: Option[BigInt])(implicit
    messages: Messages
  ): SummaryList =
    SummaryListViewModel(
      rows = Seq(
        ...
        Some(poundRow("disclosure.totals.amount", f"${totalAmounts.displayAmountDueTotal}%1.2f")),
