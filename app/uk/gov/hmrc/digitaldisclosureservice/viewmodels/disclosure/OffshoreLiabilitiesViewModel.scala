...
object OffshoreLiabilitiesViewModel extends CurrentTaxYear {
  ...
  def apply(
  ...
  )(implicit messages: Messages): OffshoreLiabilitiesViewModel = {

    val taxYears: Seq[TaxYearWithLiabilities] =
      offshoreLiabilities.taxYearLiabilities.getOrElse(Map()).values.toSeq.sortBy(_.taxYear)
    ...
    val taxYearLists: Seq[(Int, SummaryList)] = taxYears.map(year =>
      (
        ...
        taxYearWithLiabilitiesToSummaryList(year, foreignTaxCredits.get(year.taxYear.startYear.toString))
      )
    )
  ...
  def primarySummaryList(offshoreLiabilities: OffshoreLiabilities, disclosingAboutThemselves: Boolean, entity: String)(
    implicit messages: Messages
  ) =
    SummaryListViewModel(
      rows = Seq(
        offshoreLiabilities.behaviour.map(answer =>
          whyAreYouMakingThisDisclosure(answer, disclosingAboutThemselves, entity)
        ),
        offshoreLiabilities.excuseForNotNotifying.map(answer =>
          row("disclosure.offshore.reasonableExcuse", answer.excuse)
        ),
        offshoreLiabilities.excuseForNotNotifying.map(answer =>
          row("disclosure.offshore.reasonableExcuse.years", answer.years)
        ),
        offshoreLiabilities.reasonableCare.map(answer =>
        ...
        offshoreLiabilities.excuseForNotFiling.map(answer =>
        ...
        offshoreLiabilities.youHaveNotIncludedTheTaxYear.map(answer =>
          row(messages("disclosure.offshore.notIncluding", getMissingYears(offshoreLiabilities)), answer)
        ),
        offshoreLiabilities.youHaveNotSelectedCertainTaxYears.map(answer =>
          row(messages("disclosure.offshore.notIncluding", getMissingYears(offshoreLiabilities)), answer)
        ),
        offshoreLiabilities.taxBeforeFiveYears.map(answer =>
          row(
            messages("disclosure.offshore.before", getEarliestYearByBehaviour(Behaviour.ReasonableExcuse).toString),
            answer
          )
        ),
        offshoreLiabilities.taxBeforeSevenYears.map(answer =>
          row(messages("disclosure.offshore.before", getEarliestYearByBehaviour(Behaviour.Careless).toString), answer)
        ),
        offshoreLiabilities.taxBeforeNineteenYears.map(answer =>
        ...
        offshoreLiabilities.notIncludedDueToInterpretation.map(answer =>
          row("disclosure.offshore.notInc", messages(s"howMuchTaxHasNotBeenIncluded.${answer.toString}"))
        ),
        offshoreLiabilities.maximumValueOfAssets.map(answer =>
          row("disclosure.offshore.maxValue", messages(s"theMaximumValueOfAllAssets.${answer.toString}"))
        )
      ).flatten
    )
  ...
  def getMissingYears(offshoreLiabilities: OffshoreLiabilities): String = {
    val yearList =
      offshoreLiabilities.whichYears.getOrElse(Nil).collect { case TaxYearStarting(y) => TaxYearStarting(y) }.toList
    TaxYearStarting.findMissingYears(yearList).map(_.startYear + 1).mkString(", ")
  }

  def whyAreYouMakingThisDisclosure(
    answers: Set[WhyAreYouMakingThisDisclosure],
    disclosingAboutThemselves: Boolean,
    entity: String
  )(implicit messages: Messages) = {
  ...
  def legalInterpretation(answers: Set[YourLegalInterpretation])(implicit messages: Messages) = {
    val value = ValueViewModel(
      HtmlContent(
        answers
          .map { answer =>
            HtmlFormat.escape(messages(s"yourLegalInterpretation.$answer")).toString
          }
          .mkString(",<br/>")
      )
    )
  ...
  def taxYearWithLiabilitiesToSummaryList(yearWithLiabilites: TaxYearWithLiabilities, foreignTaxCredit: Option[BigInt])(
    implicit messages: Messages
  ): SummaryList = {
