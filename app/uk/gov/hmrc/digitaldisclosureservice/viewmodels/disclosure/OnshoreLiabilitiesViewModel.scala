...
object OnshoreLiabilitiesViewModel extends CurrentTaxYear {
  ...
  def apply(
    onshoreLiabilities: OnshoreLiabilities,
    disclosingAboutThemselves: Boolean,
    entity: String,
    offerAmount: Option[BigInt],
    caseflowDateFormat: Boolean
  )(implicit messages: Messages): OnshoreLiabilitiesViewModel = {

    val taxYears: Seq[OnshoreTaxYearWithLiabilities] =
      onshoreLiabilities.taxYearLiabilities.getOrElse(Map()).values.toSeq.sortBy(_.taxYear)

    val lettingDeduction                      = onshoreLiabilities.lettingDeductions.getOrElse(Map())
    val taxYearLists: Seq[(Int, SummaryList)] = taxYears.map(year =>
      (
        year.taxYear.startYear,
        taxYearWithLiabilitiesToSummaryList(year, lettingDeduction.get(year.taxYear.startYear.toString))
      )
    )

    val corporationTaxLists  = onshoreLiabilities.corporationTaxLiabilities.getOrElse(Seq()).zipWithIndex.map {
      case (ct, i) => (i + 1, corporationTaxSummaryList(ct, caseflowDateFormat))
    }
    val directorLoanLists    = onshoreLiabilities.directorLoanAccountLiabilities.getOrElse(Seq()).zipWithIndex.map {
      case (dl, i) => (i + 1, directorLoanSummaryList(dl, caseflowDateFormat))
    }
    val lettingPropertyLists = onshoreLiabilities.lettingProperties.getOrElse(Seq()).zipWithIndex.map { case (p, i) =>
      (i + 1, lettingPropertySummaryList(p, caseflowDateFormat))
    }
  ...
  def primarySummaryList(onshoreLiabilities: OnshoreLiabilities, disclosingAboutThemselves: Boolean, entity: String)(
    implicit messages: Messages
  ) =
    SummaryListViewModel(
      rows = Seq(
        onshoreLiabilities.behaviour.map(answer =>
          whyAreYouMakingThisDisclosure(answer, disclosingAboutThemselves, entity)
        ),
        onshoreLiabilities.excuseForNotNotifying.map(answer =>
          row("disclosure.offshore.reasonableExcuse", answer.excuse)
        ),
        onshoreLiabilities.excuseForNotNotifying.map(answer =>
          row("disclosure.offshore.reasonableExcuse.years", answer.years)
        ),
        onshoreLiabilities.reasonableCare.map(answer =>
          row("disclosure.offshore.reasonableCare", answer.reasonableCare)
        ),
        onshoreLiabilities.reasonableCare.map(answer =>
          row("disclosure.offshore.reasonableCare.years", answer.yearsThisAppliesTo)
        ),
        onshoreLiabilities.excuseForNotFiling.map(answer =>
          row("disclosure.offshore.notfileExcuse", answer.reasonableExcuse)
        ),
        onshoreLiabilities.excuseForNotFiling.map(answer =>
          row("disclosure.offshore.notfileExcuse.years", answer.yearsThisAppliesTo)
        ),
        onshoreLiabilities.disregardedCDF.map(answer => row("disclosure.offshore.cdf", messages("service.yes"))),
        onshoreLiabilities.youHaveNotIncludedTheTaxYear.map(answer =>
          row(messages("disclosure.offshore.notIncluding", getMissingYears(onshoreLiabilities)), answer)
        ),
        onshoreLiabilities.youHaveNotSelectedCertainTaxYears.map(answer =>
          row(messages("disclosure.offshore.notIncluding", getMissingYears(onshoreLiabilities)), answer)
        ),
        onshoreLiabilities.taxBeforeThreeYears.map(answer =>
          row(
            messages("disclosure.offshore.before", getEarliestYearByBehaviour(Behaviour.ReasonableExcuse).toString),
            answer
          )
        ),
        onshoreLiabilities.taxBeforeFiveYears.map(answer =>
          row(messages("disclosure.offshore.before", getEarliestYearByBehaviour(Behaviour.Careless).toString), answer)
        ),
        onshoreLiabilities.taxBeforeNineteenYears.map(answer =>
          row(messages("disclosure.offshore.before", getEarliestYearByBehaviour(Behaviour.Deliberate).toString), answer)
        ),
        onshoreLiabilities.memberOfLandlordAssociations.map(answer =>
          row(messages("disclosure.property.landlord"), booleanText(answer))
        ),
        onshoreLiabilities.landlordAssociations.map(answer =>
          row(messages("disclosure.property.associationMembership"), answer)
        ),
        onshoreLiabilities.howManyProperties.map(answer =>
          row(messages("disclosure.property.numberOfProperties"), answer.toString)
        )
      ).flatten
    )

  def getMissingYears(onshoreLiabilities: OnshoreLiabilities): String = {
    val yearList = onshoreLiabilities.whichYears
      .getOrElse(Nil)
      .collect { case OnshoreYearStarting(y) => OnshoreYearStarting(y) }
      .toList
    OnshoreYearStarting.findMissingYears(yearList).map(_.startYear + 1).mkString(", ")
  }

  def whyAreYouMakingThisDisclosure(
    answers: Set[WhyAreYouMakingThisOnshoreDisclosure],
    disclosingAboutThemselves: Boolean,
    entity: String
  )(implicit messages: Messages) = {
        ...
        answers
          .map { answer =>
            HtmlFormat
              .escape(
                messages(
                  if (disclosingAboutThemselves) s"whyAreYouMakingThisDisclosure.you.$answer"
                  else s"whyAreYouMakingThisDisclosure.${entity.toLowerCase}.$answer"
                )
              )
              .toString
          }
  ...
  def taxYearWithLiabilitiesToSummaryList(
    ...
    lettingDeduction: Option[BigInt]
    ...
    val rows = Seq(
      ...
      liabilities.undeclaredIncomeOrGain.map(undeclaredIncome =>
        row("disclosure.onshore.undeclaredIncomeOrGain", s"$undeclaredIncome")
      )
  ...
  def corporationTaxSummaryList(liability: CorporationTaxLiability, caseflowDateFormat: Boolean)(implicit
    messages: Messages
  ): SummaryList = {

    val penaltyAmount =
      ((liability.penaltyRate * BigDecimal(liability.howMuchUnpaid)) / 100).setScale(2, RoundingMode.DOWN)
    val totalAmount   = BigDecimal(liability.howMuchUnpaid) + penaltyAmount + BigDecimal(liability.howMuchInterest)

    val rows = Seq(
      row(
        "disclosure.onshore.accountingPeriod",
        liability.periodEnd.format(if (caseflowDateFormat) dmsDateFormatter else downloadDateFormatter)
      ),
  ...
  def directorLoanSummaryList(liability: DirectorLoanAccountLiabilities, caseflowDateFormat: Boolean)(implicit
    messages: Messages
  ): SummaryList = {
  ...
  def lettingPropertySummaryList(property: LettingProperty, caseflowDateFormat: Boolean)(implicit
    messages: Messages
  ): SummaryList = {

    val typeOfMortgage: Option[String] = property.typeOfMortgage match {
      case Some(TypeOfMortgageDidYouHave.CapitalRepayment) =>
        Some(messages("disclosure.property.mortgageType.capitalRepayment"))
      case Some(TypeOfMortgageDidYouHave.InterestOnly)     =>
        Some(messages("disclosure.property.mortgageType.interestOnly"))
    ...
    val rows = Seq(
      ...
      property.dateFirstLetOut.map(value =>
        row(
          "disclosure.property.firstLet",
          value.format(if (caseflowDateFormat) dmsDateFormatter else downloadDateFormatter)
        )
      ),
      property.stoppedBeingLetOut.map(bool => row("disclosure.property.stoppedLet", booleanText(bool))),
      property.noLongerBeingLetOut.map(value =>
        row(
          "disclosure.property.stoppedLetDate",
          value.stopDate.format(if (caseflowDateFormat) dmsDateFormatter else downloadDateFormatter)
        )
      ),
      ...
      property.didTheLettingAgentCollectRentOnYourBehalf.map(bool =>
        row("disclosure.property.collectRent", booleanText(bool))
      )
    ).flatten
