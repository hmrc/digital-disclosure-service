...
class DisclosureViewModelSpec extends AnyWordSpec with Matchers with BaseSpec with SummaryListFluency {
  ...
  "metadataList" should {
    "return populated values as rows" in {
      ...
      val countryOfYourOffshoreLiability = Map(
        "GBR" -> CountryOfYourOffshoreLiability("GBR", "United Kingdom"),
        "FRA" -> CountryOfYourOffshoreLiability("FRA", "France")
      )
      val offshoreLiabilities            =
        OffshoreLiabilities(countryOfYourOffshoreLiability = Some(countryOfYourOffshoreLiability))
      ...
      DisclosureViewModel.metadataList(
        background,
        metadata,
        caseReference,
        offshoreLiabilities,
    ...
    "return unpopulated values as empty" in {
      ...
      DisclosureViewModel.metadataList(
        ...
        OffshoreLiabilities(),
        false
      ) shouldEqual SummaryListViewModel(rows = Nil)
    }
  ...
  "backgroundList" should {
    "return populated values as rows where have you received is yes" in {
      ...
      val expected   = SummaryListViewModel(
        Seq(
          SummaryListRowViewModel(
            "disclosure.background.disclosureEntity",
            ValueViewModel(messages("notification.background.Individual"))
          ),
          SummaryListRowViewModel(
            "disclosure.background.liabilities",
  ...
  "otherLiabilitiesList" should {
    ...
    "display populated rows where you're the individual" in {
      ...
      val expectedIssues   =
        messages(s"otherLiabilityIssues.employerLiabilities") + ",<br/>" + messages(s"otherLiabilityIssues.vatIssues")
      val expected         = SummaryListViewModel(
        Seq(
    ...
    "display populated rows where you're not the individual" in {
      ...
      val expected         = SummaryListViewModel(
        Seq(
          ...
          SummaryListRowViewModel("disclosure.otherLiabilities.transfer", ValueViewModel("Some gift")),
          SummaryListRowViewModel(
            "disclosure.otherLiabilities.taxCredits.individual",
    ...
    "display populated rows where it's an estate" in {
      ...
      val expected         = SummaryListViewModel(
        Seq(
          ...
          SummaryListRowViewModel("disclosure.otherLiabilities.transfer", ValueViewModel("Some gift")),
          SummaryListRowViewModel(
            "disclosure.otherLiabilities.taxCredits.person",
            ValueViewModel(messages("service.no"))
          )
        )
      )
  ...
  "additionalList" should {
    ...
    "display populated rows" in {
      ...
      val expectedReason = messages(s"whyAreYouMakingADisclosure.govUkGuidance") + ",<br/>" + messages(
        s"whyAreYouMakingADisclosure.letterFromHMRC"
      )
      val expected       = SummaryListViewModel(
        Seq(
    ...
    "display populated rows when not the individual" in {
      ...
      val expected       = SummaryListViewModel(
        Seq(
          ...
          SummaryListRowViewModel("disclosure.additional.beforeNow", ValueViewModel("Reason why not before now")),
          SummaryListRowViewModel(
            "disclosure.additional.adviceGiven.Individual",
            ValueViewModel(messages("service.yes"))
          ),
          SummaryListRowViewModel("disclosure.additional.advice.name", ValueViewModel("Some advice giver")),
          SummaryListRowViewModel("disclosure.additional.advice.behalfOf", ValueViewModel(messages("service.no"))),
          SummaryListRowViewModel("disclosure.additional.advice.business", ValueViewModel("Some org")),
