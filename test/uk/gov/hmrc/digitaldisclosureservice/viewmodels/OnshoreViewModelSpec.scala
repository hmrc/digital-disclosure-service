...
class OnshoreViewModelSpec
  ...
  "primarySummaryList" should {
    ...
    def getYearKey(behaviour: Behaviour) =
      messages("disclosure.offshore.before", OnshoreLiabilitiesViewModel.getEarliestYearByBehaviour(behaviour).toString)
    ...
    "display populated rows when they are the individual" in {
      ...
      val expected          = SummaryListViewModel(
        Seq(
          ...
          SummaryListRowViewModel(
            "disclosure.property.associationMembership",
            ValueViewModel(HtmlContent("Some associations"))
          ),
          SummaryListRowViewModel("disclosure.property.numberOfProperties", ValueViewModel(HtmlContent("7")))
        )
      )
      OnshoreLiabilitiesViewModel.primarySummaryList(
        completeOnshoreLiabilities,
        true,
        Individual.toString
    ...
    "return zeros where the list is empty" in {
      ...
      val expected = SummaryListViewModel(
        Seq(
          ...
          SummaryListRowViewModel(
            "disclosure.onshore.undeclaredIncomeOrGain",
      ...
      OnshoreLiabilitiesViewModel.taxYearWithLiabilitiesToSummaryList(
    ...
    "display all rows" in {
      ...
      val expected = SummaryListViewModel(
        Seq(
          ...
          SummaryListRowViewModel(
            "disclosure.property.stoppedLet",
            ValueViewModel(HtmlContent(messages("service.yes")))
          ),
          ...
          SummaryListRowViewModel(
            "disclosure.property.mortgageType",
            ValueViewModel(HtmlContent("Some other type of mortgage"))
          ),
          SummaryListRowViewModel(
            "disclosure.property.lettingAgent",
            ValueViewModel(HtmlContent(messages("service.yes")))
          ),
          SummaryListRowViewModel(
            "disclosure.property.collectRent",
            ValueViewModel(HtmlContent(messages("service.no")))
          )
        )
      )
